...
function pick_and_place.convert_to_replacement_wallmounted(pos, node, groupname)
    node = node or minetest.get_node(pos)

    local meta = minetest.get_meta(pos)
    meta:set_string("group", groupname)

    if node.name == "pick_and_place:replacement_wallmounted" then
    ...
    node.name = "pick_and_place:replacement_wallmounted"
